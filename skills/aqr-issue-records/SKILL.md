...
- **Create and clarify.** When the user says to create an issue, make the directory and `task.md`, then clarify the work — goal, context, scope, acceptance criteria, and the decisions reached. Try hard to surface anything important but uncertain and capture it in Open Questions: for a bug or behavior change, reproduce the current behavior and pin down the expected one; for a non-trivial design, propose the basic direction as a candidate decision for the user to confirm. Resolve open questions as you go; `task.md` is still open, so keep refining it until initial clarification is done (Open Questions empty).
