...
- **Plan and execute (the freeze point).** Once the user says to proceed, `task.md` is frozen — do not edit it again. Create `progress.md`, then plan and execute; the *how* (replanning, self-critique, …) is up to your methodology skill (e.g. Superpowers) — this skill only pins the freeze point and the record. Decisions that surface now go in `progress.md`. Carry on to completion unless the user stops you; do not pause to ask when a reasonable default exists.
