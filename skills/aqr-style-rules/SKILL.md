...
| - | - | - |
| Code and tests (Python) | `TypedDict` over `dataclass`, `os.path` over `pathlib`, relative imports, plain test functions, exact-equality assertions, ruff / pyright / pytest toolchain, `.python-version` / `pyproject.toml` config, editorconfig | `reference/code-python.md` |
| Code and tests (JavaScript) | `.nvmrc` Node version pin, editorconfig — minimal seed | `reference/code-javascript.md` |
